...
class UploadHandler {
	...
	protected function savePageViaJob( array &$options ) {
		...
		if ( $this->jobs_added > Config::$job_throttle ) { return; }
	...
	public function saveMediawikiTemplateAsPage( array &$user_options ) {
		...
		$options['comment'] = trim( $this->user_options['comment']  . ' ' . wfMessage( 'gwtoolset-upload-on-behalf-of' )->params( $this->_User->getName() )->plain() );
	...
	protected function uploadMetadataFile() {
		...
		$comment = wfMessage( 'gwtoolset-upload-on-behalf-of' )->params( $this->_User->getName() )->plain();
